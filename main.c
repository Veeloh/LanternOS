...
void kernel_main(unsigned int multiboot_addr) {
	...
	// --- TEMP TEST: exercise fat32_write_file before cmd_write exists.
	// Remove this block once `write`/`touch` are wired into the shell.
	const char* test_data = "hello from fat32_write_file!\n";
	int wr = fat32_write_file("TEST.TXT", (const uint8_t*)test_data, 30);
	vga_print("\nfat32_write_file returned: ");
	if (wr < 0) {
		vga_print("ERROR ");
		char c = '0' - wr; // -1 -> '1', -2 -> '2'
		char buf2[2] = { c, 0 };
